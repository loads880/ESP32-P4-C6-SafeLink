...
FAULT_PATH_RE = re.compile(r"^/fault/drop/5MiB-at-1MiB\.bin$")
...
class SafeLinkHandler(BaseHTTPRequestHandler):
    ...
    server_version = "SafeLinkDownload/1.1-test6b"
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            self._send_json(200, {"service": "safelink-download", "status": "ok", "version": "1.1-test6b"})
            return
        if path == "/manifest.json":
            self._send_json(200, {
                "pattern": "bytes 0x00 through 0xFF repeated",
                "fault": {
                    "path": "/fault/drop/5MiB-at-1MiB.bin",
                    "declared_bytes": 5 * MIB,
                    "disconnect_after_bytes": MIB,
                },
                "files": [
        ...
        position = start
        send_length = min(length, MIB) if fault_drop else length
        ...
        try:
            ...
            if fault_drop:
                ...
                print(f"{self.client_address[0]} - TEST 6B forced disconnect after {send_length} of {length} bytes")
                try:
...
def main() -> None:
    ...
    print("SafeLink deterministic download server 1.1 - Test 6B fault injection")
    ...
    for address in local_addresses():
        ...
        print(f"P4 fault:    http://{address}:{args.port}/fault/drop/5MiB-at-1MiB.bin")
