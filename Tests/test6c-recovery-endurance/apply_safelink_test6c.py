#!/usr/bin/env python3
"""Apply SafeLink Test 6C to the established P4 laboratory project."""
...
def main() -> int:
    ...
    source = Path(__file__).with_name("transport_config_main.test6c.c")
    for path in (target, cmake, source):
        if not path.is_file():
            raise FileNotFoundError(path)

    backup = target.with_suffix(target.suffix + ".safelink-test6b-backup")
    if not backup.exists():
        shutil.copy2(target, backup)
    shutil.copy2(source, target)

    cmake_backup = cmake.with_suffix(cmake.suffix + ".safelink-test6b-backup")
    ...
    print("SafeLink Test 6C applied successfully")
    print(f"Project:       {project}")
    print(f"Test 6B backup:{backup}")
    print(f"Test 6C SHA256: {digest(target)}")
